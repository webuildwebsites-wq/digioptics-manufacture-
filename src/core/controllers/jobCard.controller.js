...
export const getMainReportData = async (req, res) => {
  try {
    ...
    /* =====================================================
       CREATED DATE ORDERS (DATA + TOTALS)
    ====================================================== */
    ...
    /* =====================================================
       COMPLETED DATE ORDERS (DATA + TOTALS)
    ====================================================== */
    ...
    /* =====================================================
       PARALLEL QUERIES (STATUS BREAKDOWN + PRODUCT MODE)
    ====================================================== */

    const [statusBreakdown, productModeBreakdown] = await Promise.all([

      // Transaction summary → Order status breakdown for created orders
      orderIds.length
        ? Order.aggregate([
          {
            $match: {
      ...
      // Product mode breakdown (Stock Lens vs Rx) for created orders
      orderIds.length
        ? Order.aggregate([
          {
            $match: {
    ...
    // Balance received on completed orders = totalOrderPrice - price (base lens price)
    const completedBalanceReceived =
      completedSummary.completedTotalSum - completedSummary.completedPriceSum;

    // Transaction summary shaped as { status: { totalAmount, count } }
    const transactionSummary = {};
    ...
    // Product mode summary shaped as { productMode: { totalAmount, count } }
    ...
    return res.status(200).json({
      ...
      totalAdvance: createdSummary.totalPrice,         // base lens price → was advance
      totalBalance: createdSummary.totalOrderPrice,    // full order value → was balance
      totalShippingCharges: createdSummary.totalShipping,
      totalOtherCharges: createdSummary.totalOtherCharges,

      // Completed orders (was "delivered")
      ...
      // Commission removed (no commission data in Order schema)
      commissionByCreated: [],
      totalCommissionCreated: 0,
      commissionByDelivered: [],
      totalCommissionDelivered: 0,

      // Status breakdown (was transaction summary)
      transactionSummary,

      // Extra: product mode breakdown
      productModeSummary,
    });
...
export const getDailyReportData = async (req, res) => {
  try {
    ...
    const [
      ...
      /* ===================== PRODUCTS (Category/Brand breakdown) ===================== */
      Order.aggregate([
        { $match: baseMatch },
        {
          $addFields: {
      ...
      /* ===================== JOBCARD STATUS (Order Status Breakdown) ===================== */
      Order.aggregate([
        { $match: baseMatch },
        {
          $group: {
      ...
      /* ===================== DELIVERED JOBCARDS (Completed Orders) ===================== */
      Order.aggregate([
        {
          $match: {
            ...baseMatch,
      ...
      /* ===================== SALES (All submitted/processing/completed orders) ===================== */
      Order.aggregate([
        {
          $match: {
            ...baseMatch,
